...
- **El volumen de InfluxDB no es el recurso escaso; el barrido de consultas sí** (medido 2026-08-09, a raíz de la duda por el volumen que genera live mode). Con live corriendo 7 h en los días soleados el ritmo pasa de ~1.350 a ~5.760 payloads/día — **4,3×**, no 12×, porque live se auto-limita a las horas de excedente. A 26 campos y con la compresión TSM sobre floats de variación lenta son **~170 MB/año, ~1,7 GB en 10 años**: sobre el disco de 4 TB del NAS, 0,04%. Aunque la estimación se equivoque por 10×, sigue siendo irrelevante.

  Lo que sí escala mal con los años es **escanear millones de puntos de 5 s para un gráfico de rango largo**. El disparador a vigilar es ese, no el disco: **si un dashboard de rango largo empieza a tardar, la respuesta es una tarea de downsampling** (agregados de 1 min o 1 h para rangos largos, crudo sólo para lo reciente), no borrar datos ni achicar live mode. Hoy no hace falta.

  Dos cosas más del mismo relevamiento: la **retención del bucket es infinita**, que para una estación meteorológica es probablemente lo correcto —la serie histórica larga *es* el producto— pero conviene que sea una elección consciente. Y la **cardinalidad crece por el tag `firmware`**, una serie nueva por versión para siempre (28 al 2026-08-09). Irrelevante en capacidad —InfluxDB aguanta millones— pero es el tag que ya rompió cinco queries con `last()` devolviendo un resultado por serie; toda query nueva tiene que contemplarlo.
